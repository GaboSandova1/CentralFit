...
export default function ConfirmMemberDeletionModal({ isOpen, onClose, memberName = 'Alex Mercer', memberId = '25438096' }: ConfirmMemberDeletionModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[2px] p-4">
      ...
      <div className="bg-surface border border-outline-variant rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.5)] w-full max-w-md overflow-hidden animate-[fadeIn_0.2s_ease-out]">
        ...
        <div className="p-4 pt-4 flex gap-4 justify-end bg-surface">
          <button 
            onClick={onClose}
            className="flex-1 px-4 py-2 rounded border border-outline-variant text-on-surface-variant font-label-md text-label-md hover:bg-surface-container-high hover:text-on-surface transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-surface cursor-pointer" 
            type="button"
          >
            Cancelar
          </button>
          <button 
            onClick={onClose}
            className="flex-1 px-4 py-2 rounded bg-error text-on-error font-label-md text-label-md hover:bg-error/90 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-error focus:ring-offset-2 focus:ring-offset-surface cursor-pointer" 
            type="button"
          >
            Eliminar
          </button>
