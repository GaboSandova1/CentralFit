    ...
    <>
      <header className="h-topbar-height fixed top-0 right-0 left-0 md:left-sidebar-width z-20 bg-surface dark:bg-surface border-b border-outline-variant dark:border-outline-variant flex justify-between items-center px-gutter w-full md:w-[calc(100%-var(--spacing-sidebar-width))]">
        <div className="flex items-center gap-3">
          <button onClick={onMenuClick} className="md:hidden text-on-surface-variant hover:text-primary transition-colors cursor-pointer" aria-label="Abrir menú">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => setNotificationsOpen((prev) => !prev)} className="text-on-surface-variant hover:text-primary transition-colors relative cursor-pointer">
                  ...
                  <div className="max-h-72 overflow-y-auto">
                    {attentionList.length === 0 ? (
                      ...
                      attentionList.map((m) => {
                        ...
                        return (
                          <button key={m.id} onClick={() => handleNotificationClick(m)} className="w-full px-4 py-2.5 border-b border-outline-variant/50 flex items-center gap-2 hover:bg-surface-container-high/50 transition-colors text-left cursor-pointer">
